    ...
    gradx = {
            ...
            if (this.targets.length > 0) {
                ...
                for (i = 0; i < len; i++) {
                    ele = gradx.gx(this.targets[i]);
						if (jQuery(ele).children(".bgcolor").length>0) jQuery(ele).children(".bgcolor").val("");
                    for (j = 0; j < v_len; j++) {
                        ele.css("background-image", values[j]);
						if (jQuery(ele).children(".bgcolor").length>0) jQuery(ele).children(".bgcolor").val(jQuery(ele).children(".bgcolor").val()+values[j]+";");
                    }
        ...
        load_gradx: function(id, sliders) {
            ...
            var html = "<div class='gradx'>\n\
						...
						<div id='gradx_close' class='gradx_remove_slider gradx_btn'><i class='icon icon-remove'></i>close</div>\n\
